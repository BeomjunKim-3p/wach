...
export module primitive.register_file_redge;

import primitive.register_redge;
...
export namespace thpg::primitive {
	...
	class RegisterFileREdge : public sc_module {
	public:
		...
		RegisterFileREdge(sc_module_name name, sc_trace_file *tf = nullptr);

		void refreshcs(void);
	
	private:
	// 대충 rcs만 demux통과시키고 나머지 다 묶기
		sc_vector<RegisterREdge<N>> rregister{"R REGISTER", W};
		sc_vector<sc_signal<sc_logic>> rcs{"R CS", W};
		sc_vector<sc_signal<sc_lv<N>>> ralwaysout{"R ALWAYSOUT", W};
	};
};

#include "register_file_redge.ipp"
